...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        #    and continue working on the problem.
        # --------------------------------------------------------------
        total_string = self.contents + additional_contents
        ...
        if (len(self.contents) + len(additional_contents)) <= self.volume:
            ...
            for k in range(self.volume):
                keep += str(total_string[k])
            self.contents = keep

            extra = ''
            for j in range(self.volume, len(total_string)):
                extra += str(total_string[j])
    ...
    def double_then_shrink(self, new_volume):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        double_discard = self.double()

        shrink_discard = self.shrink(new_volume)

        return len(double_discard + shrink_discard)
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        copy_self = Box(self.contents, self.volume)

        copy_self.volume += other_box.volume
        copy_self.append_string(other_box.contents)

        return Box(copy_self.contents, copy_self.volume)
